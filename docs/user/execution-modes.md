...
`bash` and subprocess tools receive identical `readOnlyProject` treatment because the executor resolves the sandbox decision exactly once per tool call, in `internal/tool.Executor.runPipeline`, and both dispatch paths consume that same resolved decision (see [Tool sandboxing](sandboxing.md#sandbox-wrapper-resolution)). Earlier, `bash` and subprocess tools computed their sandbox mode independently, which let a config-defined subprocess tool run with an unrestricted (writable) project mount in plan mode while `bash` was correctly read-only.
