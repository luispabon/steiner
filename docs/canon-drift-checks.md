...
Go tests catch a downstream instruction file drifting away from the orchestration canon, `delegationInstructions` in `internal/prompt/system.go`, and keep the parent/child workflow boundary explicit. This doc explains what the checks do, what counts as canon and as a "consumer," and what to do when one fails.

Delegation canon prose lives in `internal/prompt/templates/delegation.md.tmpl`, a `text/template` embedded into the binary via `embed.FS`. `delegationInstructions()` in `internal/prompt/system.go` executes it: prose is edited in the template file, never in Go string constants. Parent and delegated-child methodology lives separately in `workflow_approval.md.tmpl`; children receive its shared editing, verification, and final-response rules plus delegated authorization, but never parent planning or delegation canon.

The specialist roster itself is not hand-written markdown. `internal/prompt/specialists.go` holds it as a typed `[]specialist` slice, and the `## Your sub-agents` table is rendered from that slice — `specialistViews()` projects it into the template data and `delegation.md.tmpl` ranges over it to emit the table rows. Advisor guidance is rendered only by its separately gated template, not by delegation canon. The table therefore cannot diverge from the roster source, and neither check needs to parse markdown to recover it.
...
- **Gated-tool reference check** (`internal/prompt/system_test.go`, `TestDelegationCanonDoesNotNameAdvisorWhenDisabled`). Advisor guidance lives only in its separately gated advisor preamble. Delegation canon must not name `advisor` when advisor guidance is disabled, and the test asserts the disabled variant contains no backticked `advisor` in either the normal or override preamble path. Canon would otherwise point the orchestrator at a tool that is not registered in a `delegation.enabled` + `advisor.enabled: false` session. Runs as part of `go test ./internal/prompt/`.
