...
|------------|-------------------------------------------------------------|
| `explore`  | `read`, `glob`, `grep`, `ls`, `bash` (read-only project sandbox), `lsp_definitions`, `lsp_references`, `lsp_diagnostics`† |
| `research` | `read`, `glob`, `grep`, `ls`, `web_search`\*, `fetch_url`\* |
| `code`     | `read`, `glob`, `grep`, `ls`, `mutate`, `bash`, `advisor`, `lsp_definitions`, `lsp_references`, `lsp_diagnostics`† |
| `evaluate`    | `read`, `glob`, `grep`, `ls`, `advisor`                     |
| `sanity_check`| `read`, `glob`, `grep`, `ls`, `bash`                        |
| `vision`   | `read`                                                      |
| `review`      | `read`, `glob`, `grep`, `ls`, `bash`, `advisor`, `lsp_definitions`, `lsp_references`, `lsp_diagnostics`† |
...
† `lsp_definitions`, `lsp_references`, and `lsp_diagnostics` require `lsp.enabled` and a configured language server for the file's extension. When `lsp.enabled: false`, these tools are not registered and agents cannot access them.
