...
User-facing documentation: [Context Management](../user/context-management.md).
...
Tool-result content that already reached the provider is frozen. Such messages carry `Message.Ingested`, and post-ingestion retains their bytes verbatim instead of reshaping them; for `read` results it still restores read-tracking state via `ObserveRead` with annotations disabled, so the tracker is rebuilt without rewriting the historical bytes or emitting annotation diagnostics. Unmarked legacy messages are shaped once and then marked `Ingested`. This keeps the prompt-cache prefix stable when a session resumes against files that have since changed on disk.
