...
It attributes parent gaps to `no_prior_baseline`, `prefix_rewrite`, `delegation`, `idle`, or `unexplained`. `no_prior_baseline` outranks the rest and means the payload's `prefix_predecessor_known` is explicitly `false`: the producer had no in-process predecessor to compare the request's prefix against, so `shared_prefix_messages` carries no rewrite signal. A record that omits the field predates it and keeps the `prefix_rewrite` reading, which follows and outranks delegation because a divergence at message 0 cannot match any cached entry. Delegation and idle use a four-minute threshold. `no_prior_baseline` is not `cold_start`: it marks a missing in-process predecessor on any turn, while `cold_start` still latches once per process on the first usage-bearing record.
