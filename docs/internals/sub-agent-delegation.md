User-facing documentation: [Sub-agent Delegation](../user/sub-agent-delegation.md).
...
The parent's execution-mode getter (`config.ExecutionMode`) is threaded the same way, as `ModeGetter`: `DelegateDeps` → `SubAgentHandlerDeps` → `childRunRequestParams`, and `buildChildRunRequest` wires it into the child executor via `WithModeGetter`. This is what makes a child's own `runPipeline` see plan mode when the parent is in plan mode, restricting the child's `mutate` calls to `.steiner/plans/` and `.steiner/security/` and mounting the project read-only for the child's `bash`/subprocess calls — matching the enforcement a parent-issued command would get, without any per-agent-type special-casing beyond the existing `readOnlyBash` flag for explore and evaluate children.
...
**3a. Per-child advisor budget.** For `code`, `review`, and `evaluate` agent types, `BuildChildRun` registers the advisor tool with a per-child budget state. `AdvisorBudgetStore` (keyed by agent ID) vends a `SharedState` for each child, ensuring they never share an advisor budget with the parent or each other. The budget survives `follow_up` resumption because the same agent ID retrieves the same `SharedState`. When `DelegateDeps.AdvisorBudgetStore` is nil or advisor is disabled, no child receives the advisor tool. The per-child cap is controlled by `config.AdvisorConfig.MaxUsesPerSubAgent`, distinct from the parent's `MaxUsesPerRun`.
...
`StopReasonUsageLimit` maps to `StatusFailed`. A usage-limit failure means the provider reported a quota or usage limit (e.g., Codex `usage_limit_reached`, OpenAI `insufficient_quota`, LiteLLM budget exhaustion); these errors are never retried at any layer. The failure `reason` carries the provider's usage-limit message (e.g., "provider usage limit reached (codex/luna/high, plan: plus): The usage limit has been reached — resets at 15:34 local (in 1h9m42s)") followed by a fixed sentence: "To continue now, delegate a fresh sub-agent; new sub-agents use the currently configured model. If that also reports a usage limit, stop delegating and tell the user." Each agent fails independently — steiner mixes providers and models across the orchestrator and sub-agents, so one agent's limit says nothing about the others. `follow_up` stays pinned to the child's original provider/model; resumability after the limit resets is tracked in issue [#784](https://github.com/luispabon/steiner/issues/784).
...
1. Validates the structured task fields (`objective`, `context`, `deliverable`, `constraints`, `success_criteria`, `checks`, all required; the first three must be non-empty after trimming) and `image_id` input.
2. Looks up the `ImageRef` from `ImageStore` (registered when the image was pasted).
3. Reads the image file from the conversation's folder under `.steiner/tmp/images/<session id>/` and base64-encodes it. An unknown ID yields a "not registered" error, and a missing or empty file path yields a "paste it again" error.
...
7. Saves the child session so the parent model can use `follow_up` for additional questions about the same image; each vision follow-up reattaches the stored `Spec.Images` to its appended final user message, since the provider keeps no server-side image cache.
...
Each delegated child gets its own `agent.VisionCapabilities` tracker, derived from its resolved model alias and `Vision` state with `SubAgentConfigured=false`. A child never inherits the parent session's vision state and cannot nest vision routing: when a child's `read` tool returns an image, that payload reaches exactly the child model request immediately following the read and is stripped from later requests — the same one-request contract as the parent, minus routing.

**ImageStore** is a goroutine-safe, conversation-scoped registry in `internal/agent/image_store.go`. `NewImageStore(root)` starts unbound on an ephemeral `.steiner/tmp/images/proc-<pid>-<hex>/` folder; `BindSession(sessionID, minNext)` switches it to `.steiner/tmp/images/<session id>/`, loading that folder's `index.json` and setting the next ID to the largest of the index's `next`, the highest indexed ID + 1, and `minNext`. Interactive sessions bind at construction, on `/new`, and on resume (with the floor from the loaded lineage); forks call `CopySession` before binding, which copies the folder, rewrites index paths, and refuses a non-empty destination. Folders are created lazily on the first `Register`, which assigns the ID and persists the index atomically. It maps auto-assigned IDs (`img-1`, `img-2`, …) to `ImageRef` values (file path, media type, dimensions, size). Pasted-image files are owned by the store, saved under the conversation's session folder (`.steiner/tmp/images/<session id>/`) with `YYYYMMDD_HHMMSS_<hex>.ext` filenames, and not deleted on agent exit (`Cleanup()` removes only the ephemeral per-process folder). Stale folders untouched for 30 days are pruned best-effort when the store is created. Read-produced images may also be registered, but their files can be externally stored under `.steiner/tmp/fetched/` or in repository paths; `Remove` deletes a file only when it lies inside the current folder, and `ImageStore.Cleanup()` does not delete those files. `.steiner/tmp/fetched/` has its own age- and budget-based retention sweep (`builtin.PruneFetchedDir`, run at startup); `.steiner/tmp/images/` is untouched by it. `ImageStore` is wired from the composition root through `DelegateDeps.ImageStore` → `SpecializedToolDeps.ImageStore` → vision handler.
...
The system-prompt canon is maintained in `internal/prompt/system.go` (`delegationInstructions`) and the specialist roster in `internal/prompt/specialists.go`. The parent preamble casts the model as orchestrator and supplies a numbered workflow: investigate with `explore`, ask clarifying questions, research, summarize Goal/Assumptions/Scope/Unknowns, plan, split work into single-logical-unit steps, dispatch one cold `code` child per step, review once, amend through the implementing child with re-review where eligible, and finish with `sanity_check`. When enabled, `advisor` is an inline workflow step and its own preamble section provides the detail. Local work is limited to genuinely self-contained bounded lookups, formatting, or a tiny user-directed correction with exact replacement text; other work is delegated.

`skills/review/SKILL.md` and `skills/simplify/SKILL.md` deliberately allow a last-resort inline-fixes tier when delegation tooling is unavailable. `skills/implement/SKILL.md` and `internal/oneshot/prompts/implement.md` forbid direct mutation of implementation-scoped files, while `internal/oneshot/prompts/review.md` has no inline-fix tier. These are deliberate consumer-file divergences from the system prompt canon.

When delegation is enabled, the block is gated on `delegationEnabled`, not workflow mode. Oneshot phases use delegated-child workflow mode but still orchestrate and therefore receive the role; delegated children do not set `DelegationEnabled` and do not receive it. `delegationEnabled` and `advisorEnabled` are cache-key inputs to `CachedSystemPreamble`, so this gate does not add per-turn prefix nondeterminism.
