# Documentation

Steiner is a local-first Go coding agent with bounded context and sandboxed execution.

## Get started

- [Getting started](user/getting-started.md)
- [Provider and model setup](user/provider-and-model-setup.md)

## User documentation

- [CLI reference](user/cli.md)
- [Configuration reference](user/configuration.md)
- [Built-in tools](user/tools.md)
- [Updates](user/updates.md)
- [Context management](user/context-management.md)
- [Execution modes](user/execution-modes.md)
- [Sub-agent delegation](user/sub-agent-delegation.md)
- [Oneshot mode](user/oneshot.md)
- [Model enumeration](user/model-enumeration.md)
- [Sandboxing](user/sandboxing.md)
- [MCP](user/mcp.md)
- [LSP](user/lsp.md)
- [Advisor](user/advisor.md)
- [Cache stats](user/cache-stats.md)
- [Optional features](user/optional-features.md)
- [Desktop notifications](user/desktop-notifications.md)
- [Images](user/images.md)
- [TUI background preferences](user/tui-background.md)

## Internals and maintenance

- [Internals](internals/index.md)
- [Canon drift checks](maintenance/canon-drift-checks.md)

## Research

- [Gap report and research milestones](research/gap-report-research-milestones.md)
- [Image handling research](research/image-handling.md)

The GitHub Wiki publishes the user and internals pages for browsing outside the repository.
