...
- **Sidebar.** A compact `MCP [spinner] <connected>/<total>` row in the status block. While any server is connecting or reconnecting, a braille spinner animates before the count; once all servers settle, the spinner disappears. The count is error-styled when any server has failed, and the entire row is hidden — not shown as "MCP: off" — when MCP is off or nothing is configured (`internal/tui/sidebar_mcp.go`; `internal/tui/sidebar_sections.go:109-141`). Disabled servers are excluded from both numbers (`internal/tui/model.go:254-275`). In interactive mode the row updates as status events arrive, and the ticker keeps the spinner animating even before the first user message (`internal/tui/model.go:521-529`; `internal/tui/model_update.go:201`). The spinner enables live visibility into the connection lifecycle at startup without requiring a user interaction to observe it.
...
- **Startup and transition warnings.** The interactive TUI emits one warning line per failed server plus an aggregate line, at the transition, deduplicated per failure generation (`internal/tui/mcp_warnings.go:14-72`). Initial-connect transitions are now visible in the sidebar as the connection lifecycle unfolds, live from startup. The MCP snapshot driven by `mcp_status` events is states-only (no tool defs) before the first turn, so the sidebar spinner animates independently of tool registration (`internal/mcp/state.go:12-25`; `cmd/steiner/interactive_session.go:415-440`). A server may therefore briefly display as `connected` in the sidebar before its tool defs are registered — this relaxes the prior "never observes half-connected" invariant to a states-only window. The MCP overlay remains coherent during this window because it reads advertised tool lists from the snapshot (`AdvertisedTools`), not the registry (`internal/tui/mcp_overlay.go:70-111`).
