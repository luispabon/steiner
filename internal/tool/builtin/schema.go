...
func MutateSchema() map[string]any {
	operationSchema := map[string]any{
		...
		"additionalProperties": false,
		"properties": map[string]any{
			"type":        map[string]any{"type": "string", "enum": []string{"create", "write", "replace", "line_replace", "delete", "move", "insert_before", "insert_after"}, "description": "Operation type"},
			...
			"file_hash":   map[string]any{"type": "string", "description": "Content hash from read output; when provided, the operation fails if the file has changed since the hash was computed"},
