...
func TestChallengeS256(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "challenge is correctly computed from verifier",
			verifier: base64.RawURLEncoding.EncodeToString(make([]byte, 32)),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ChallengeS256(tt.verifier)

			// Verify the challenge is correct by checking it matches SHA256 of verifier
