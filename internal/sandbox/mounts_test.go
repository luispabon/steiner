...
func TestBuildArgs_WorkspaceBind(t *testing.T) {
	args := BuildArgs("/my/workspace", "/my/workspace", "/my/workspace/.steiner/home", "/home/user", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_SeparateWritableRootAndWorkDir(t *testing.T) {
	args := BuildArgs("/repo", "/repo/.git-worktrees/step-1a", "/repo/.steiner/home", "/home/user", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_SandboxHomeBind(t *testing.T) {
	...
	args := BuildArgs("/my/workspace", "/my/workspace", sandboxHome, "/home/user", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_UnshareAll(t *testing.T) {
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", "/home/user", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_HostMountsAppended(t *testing.T) {
	...
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", "/home/user", hostMounts, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_CacheDirBind(t *testing.T) {
	...
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", home, nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, true)
...
func TestBuildArgs_NoCacheDirWhenMissing(t *testing.T) {
	...
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", home, nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_NoCacheDirWhenNoHome(t *testing.T) {
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", "", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_AppendsOverlayBeforeChdir(t *testing.T) {
	...
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", "/home/user", nil, overlayArgs, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)
...
func TestBuildArgs_DieWithParentAndNewSession(t *testing.T) {
	args := BuildArgs("/ws", "/ws", "/ws/.steiner/home", "/home/user", nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, false)

	for _, flag := range []string{"--die-with-parent", "--new-session"} {
...
func TestBuildArgs_CacheBinding(t *testing.T) {
	...
	sandboxHome := "/ws/.steiner/home"
	private := privateCacheDir(sandboxHome)
	...
	tests := []struct {
		...
		{"default uses private cache", false, private},
		{"opt-in binds host cache", true, resolved},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			args := BuildArgs("/ws", "/ws", sandboxHome, home, nil, nil, "/tmp/sandbox-tmp", false, config.PermissionsConfig{}, tt.bindHostCache)
			if !containsSeq(args, "--bind", tt.wantSrc, resolved) {
				t.Errorf("expected --bind %s %s in args: %v", tt.wantSrc, resolved, args)
			}
			hostRW := containsSeq(args, "--bind", resolved, resolved)
			if hostRW != tt.bindHostCache {
				t.Errorf("real cache rw bind = %v, want %v", hostRW, tt.bindHostCache)
			}
		})
	}
}
