...
func NewWorktreeCleanupPlan(list func() (int, error), prune func(context.Context) (int, error)) *WorktreeCleanupPlan {
	return &WorktreeCleanupPlan{list: list, prune: prune}
}

// Count returns the number of worktrees available for cleanup.
func (p *WorktreeCleanupPlan) Count() (int, error) {
	if p == nil || p.list == nil {
		return 0, nil
	}
	return p.list()
