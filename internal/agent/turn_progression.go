...
func (p *turnProgressor) executeModelCall(ctx context.Context, state RunState, assembly prompt.Assembly, chatRequest provider.ChatRequest) (turnOutcome, *provider.ChatResponse) {
	...
	state, turnTokens := p.finalizeModelCallState(state, turn, chatRequest, response)
...
func (p *turnProgressor) finalizeModelCallState(state RunState, turn int, chatRequest provider.ChatRequest, response provider.ChatResponse) (RunState, int) {
