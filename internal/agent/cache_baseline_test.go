...
func TestCacheBaselineStore_EmptyKeyIsNoOp(t *testing.T) {
	...
	if shared, known := store.Compare(CacheBaselineKey{}, []string{"a"}); shared != 0 || known {
		t.Fatalf("Compare(empty key) = (%d, %v), want (0, false)", shared, known)
	}
	store.Promote(CacheBaselineKey{}, []string{"a"})
...
func TestCacheBaselineStore_NilReceiverIsNoOp(t *testing.T) {
	...
	if shared, known := store.Compare(CacheBaselineKey{CacheKey: "k"}, []string{"a"}); shared != 0 || known {
		t.Fatalf("Compare on nil store = (%d, %v), want (0, false)", shared, known)
	}
	store.Promote(CacheBaselineKey{CacheKey: "k"}, []string{"a"})
}

func TestCacheBaselineStore_CompareDoesNotMutateState(t *testing.T) {
	store := NewCacheBaselineStore()
	key := CacheBaselineKey{CacheKey: "k"}
	store.Promote(key, []string{"a", "b"})

	for i := 0; i < 3; i++ {
		if shared, known := store.Compare(key, []string{"a", "b", "c"}); !known || shared != 2 {
			t.Fatalf("Compare() call %d = (%d, %v), want (2, true)", i, shared, known)
		}
	}
}

func TestCacheBaselineStore_PromoteThenCompareReportsPromotedSequence(t *testing.T) {
	store := NewCacheBaselineStore()
	key := CacheBaselineKey{CacheKey: "k"}

	if shared, known := store.Compare(key, []string{"a"}); known || shared != 0 {
		t.Fatalf("Compare() before any promotion = (%d, %v), want (0, false)", shared, known)
	}

	store.Promote(key, []string{"a", "b"})
	if shared, known := store.Compare(key, []string{"a", "b", "c"}); !known || shared != 2 {
		t.Fatalf("pure append after promotion = (%d, %v), want (2, true)", shared, known)
	}

	store.Promote(key, []string{"a", "x", "c"})
	if shared, known := store.Compare(key, []string{"a", "x", "z"}); !known || shared != 2 {
		t.Fatalf("rewrite at index 2 after promotion = (%d, %v), want (2, true)", shared, known)
	}
}
...
func TestCacheBaselineStore_IdentityIsolation(t *testing.T) {
	...
	for name, mutate := range map[string]func(CacheBaselineKey) CacheBaselineKey{
		...
		"agent id":        func(k CacheBaselineKey) CacheBaselineKey { k.AgentID = "agent-b"; return k },
		...
		t.Run(name, func(t *testing.T) {
			if _, known := store.Compare(mutate(base), []string{"a"}); known {
				t.Fatalf("a differing %s must not share the baseline", name)
			}
		})
	}

	if shared, known := store.Compare(base, []string{"a"}); !known || shared != 1 {
...
func TestCacheBaselineStore_AppendAndDivergence(t *testing.T) {
	...
	if _, known := store.Compare(key, []string{"a", "b"}); known {
	...
	store.Promote(key, []string{"a", "b", "c"})

	if shared, known := store.Compare(key, []string{"a", "x", "c"}); !known || shared != 1 {
		t.Fatalf("rewrite at index 1 = (%d, %v), want (1, true)", shared, known)
	}
	store.Promote(key, []string{"a", "x", "c"})

	if shared, known := store.Compare(key, []string{"z"}); !known || shared != 0 {
...
func TestCacheBaselineStore_EmptySequenceIsAKnownPredecessor(t *testing.T) {
	store := NewCacheBaselineStore()
	key := CacheBaselineKey{CacheKey: "k"}
	if _, known := store.Compare(key, nil); known {
		t.Fatal("first request reported a predecessor, want none")
	}
	store.Promote(key, nil)
	if shared, known := store.Compare(key, nil); !known || shared != 0 {
...
func TestCacheBaselineStore_ConcurrentUse(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 100; j++ {
				store.Compare(key, []string{"a", "b"})
				store.Promote(key, []string{"a", "b"})
				store.Compare(CacheBaselineKey{CacheKey: "other"}, []string{"c"})
				store.Promote(CacheBaselineKey{CacheKey: "other"}, []string{"c"})
			}
		}()
	}
	wg.Wait()
	...
	if shared, known := store.Compare(key, []string{"a", "b"}); !known || shared != 2 {
