...
func TestAgentAllowedTools(t *testing.T) {
	...
	t.Run("explore has bash and lsp tools but no mutation tools", func(t *testing.T) {
		...
		for _, lspTool := range []string{"lsp_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover"} {
	...
	t.Run("evaluate has no mutation tools and no lsp tools", func(t *testing.T) {
		...
		for _, m := range append(legacyMutationTools, "bash", "mutate", "lsp_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover") {
			if slices.Contains(tools, m) {
	...
	t.Run("sanity_check has bash but not mutation tools or lsp tools", func(t *testing.T) {
		...
		for _, m := range append(legacyMutationTools, "mutate", "lsp_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover") {
			if slices.Contains(tools, m) {
	...
	t.Run("code has mutate, bash, and lsp tools", func(t *testing.T) {
		...
		for _, m := range []string{"mutate", "bash", "lsp_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover"} {
			if !slices.Contains(tools, m) {
