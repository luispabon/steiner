...
func TestAgentAllowedTools(t *testing.T) {
	...
	t.Run("sanity_check has bash but not mutation tools or lsp tools", func(t *testing.T) {
		...
		for _, m := range append(legacyMutationTools, "mutate", "lsp_definitions", "lsp_implementations", "lsp_type_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover", "lsp_symbols") {
			if slices.Contains(tools, m) {
	...
	t.Run("code has mutate, bash, and lsp tools", func(t *testing.T) {
		...
		for _, m := range []string{"mutate", "bash", "lsp_definitions", "lsp_implementations", "lsp_type_definitions", "lsp_references", "lsp_diagnostics", "lsp_hover", "lsp_symbols"} {
			if !slices.Contains(tools, m) {
