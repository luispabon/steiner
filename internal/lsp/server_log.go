...
// ServerLogPath derives the LSP server log path from the main session log path.
...
// NewServerLogWriter creates an io.WriteCloser for LSP server stderr.
...
func NewServerLogWriter(path string) (io.WriteCloser, error) {
	...
	if _, statErr := os.Stat(dir); statErr == nil {
		...
		return nil, fmt.Errorf("stat lsp server log directory: %w", statErr)
	}
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return nil, fmt.Errorf("create lsp server log directory: %w", err)
	}
	if !dirExisted {
		if err := os.Chmod(dir, 0o700); err != nil {
			return nil, fmt.Errorf("secure lsp server log directory: %w", err)
		}
	}
	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600)
	if err != nil {
		return nil, fmt.Errorf("open lsp server log: %w", err)
	}
	if err := f.Chmod(0o600); err != nil {
		_ = f.Close()
		return nil, fmt.Errorf("secure lsp server log: %w", err)
	}
	return f, nil
}
