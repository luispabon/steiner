...
- `definitions` — jump to symbol definitions; requires `lsp.enabled` and a configured language server for the file's extension (requires `lsp.enabled`)
- `references` — find all references to a symbol; requires `lsp.enabled` and a configured language server for the file's extension (requires `lsp.enabled`)
- `diagnostics` — get diagnostics for a file; requires `lsp.enabled` and a configured language server for the file's extension (requires `lsp.enabled`)
