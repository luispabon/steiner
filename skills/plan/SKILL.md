...
**Delegation.** If research is approved, delegate it — do not substitute the planner's own reasoning. Call Steiner's `sub_agent` tool with type `research` and a self-contained brief framed around the exact questions to answer. When delegation is available, follow the briefing template in your system prompt. If `sub_agent` type `research` is unavailable, report that and ask whether to continue without it or use the best available fallback. Research is complete once the delegated result is received and reviewed.
